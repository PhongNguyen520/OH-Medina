...
public static class ApifyHelper
{
    ...
    /// <summary>
    /// Tries to get input from Apify only: APIFY_INPUT_VALUE then Key-Value Store API.
    /// Returns null if not on Apify or fetch fails (so caller can fallback to local input.json).
    /// </summary>
    ...
    /// <summary>Returns input JSON from Apify only (env or KV API), or null.</summary>
